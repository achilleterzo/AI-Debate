...
export function formatHistoryMessage({ message, actor, participants = [] }) {
  ...
  // What a participant deliberated is not what a participant said. The stream
  // already keeps a leaked reasoning block out of the message, but a transcript
  // written before that, or restored from an older snapshot, still carries one
  // inside `content` — and this function is what every participant reads. It is
  // stripped here so no monologue can travel to the table as a contribution.
  const content = visibleContribution(message.content)

  if (message.role === 'topic') return asUser(`${idMarker(message)}[Topic]: ${content}`)
  if (message.role === 'participant_joined' || message.role === 'participant_left') return null
  if (message.role === 'user') return asUser(`${idMarker(message)}[Moderator]: ${content}`)
  if (message.role === 'interjection') return asUser(`${idMarker(message)}[Topic update]: ${content}`)
  ...
  if (content.startsWith('<function_calls>')) return null
  ...
  // spend their own turns asking who has not spoken yet. A turn that was
  // nothing but deliberation counts as one of those.
  if (!content) return null

  if (message.role === actor.tag) return { role: 'assistant', content }
