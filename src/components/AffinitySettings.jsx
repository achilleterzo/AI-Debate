...
export default function AffinitySettings({ randomTurnOrder, onRandomTurnOrderChange, dynamicAffinity, onDynamicAffinityChange, moderationCooling, onModerationCoolingChange, running }) {
  ...
  return (
    <div style={{ ...styles.settingsRow, flexWrap: 'wrap' }}>
      <div
        onClick={() => !running && onRandomTurnOrderChange(!randomTurnOrder)}
        title={randomTurnOrder ? ui.randomTurnOrderTitleOn : ui.randomTurnOrderTitleOff}
        style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: running ? 'default' : 'pointer', userSelect: 'none', opacity: running ? 0.5 : 1 }}
      >
        <div style={{ width: 32, height: 16, borderRadius: 8, position: 'relative', background: randomTurnOrder ? '#fb923c' : '#444', transition: 'background 0.2s', flexShrink: 0 }}>
          <div style={{ position: 'absolute', top: 2, left: randomTurnOrder ? 18 : 2, width: 12, height: 12, borderRadius: '50%', background: '#fff', transition: 'left 0.2s' }} />
        </div>
        <span style={{ fontSize: 12, color: randomTurnOrder ? '#fb923c' : '#666', whiteSpace: 'nowrap' }}>{ui.randomTurnOrder}</span>
      </div>
      <div
