  ...
  return (
    <div onClick={onCancel} style={{ position: 'fixed', inset: 0, background: '#000000bb', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#141414', border: '1px solid #2e2e2e', borderRadius: 10, width: 'min(92vw, 460px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '10px 14px', borderTop: '1px solid #2e2e2e', flexWrap: 'wrap' }}>
          ...
          {(state.options || [{ label: state.confirmLabel || ui.defaultConfirm, danger: state.danger }]).map((option, index) => (
            <button
              key={option.label || index}
              onClick={() => onConfirm(option)}
              style={{
                background: option.danger ? '#3a1e1e' : '#1f2a3f',
                border: `1px solid ${option.danger ? '#7a3b3b' : '#3f5a8a'}`,
                color: option.danger ? '#fca5a5' : '#9fc2ff',
                borderRadius: 6,
                padding: '5px 12px',
                cursor: 'pointer',
                fontSize: 12,
